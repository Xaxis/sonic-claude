...
export function Controls() {
  ...
  return (
    <div className="h-full flex flex-col p-4 space-y-5 overflow-y-auto">
        {/* Transport Controls */}
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs text-muted-foreground uppercase tracking-[0.15em] font-medium">
        ...
        <div className="space-y-2 p-3 rounded-lg bg-primary/5 border border-primary/10">
          <label className="text-xs text-muted-foreground uppercase tracking-[0.15em] font-medium">BPM</label>
          <Slider value={bpm} min={60} max={180} step={1} onChange={handleBPMChange} />
          <div className="text-center text-2xl font-bold text-primary font-mono tracking-wider">{bpm}</div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1.5 p-2 rounded-md bg-secondary/5 border border-secondary/10">
              <label className="text-xs text-muted-foreground uppercase tracking-wider">Intensity</label>
              <Slider value={intensity} min={0} max={10} step={0.1} onChange={handleIntensity} />
              <div className="text-center text-sm font-mono text-secondary font-bold">{intensity.toFixed(1)}</div>
            </div>
            <div className="space-y-1.5 p-2 rounded-md bg-secondary/5 border border-secondary/10">
              <label className="text-xs text-muted-foreground uppercase tracking-wider">Cutoff</label>
              <Slider value={cutoff} min={50} max={130} step={1} onChange={handleCutoff} />
              <div className="text-center text-sm font-mono text-secondary font-bold">{cutoff}</div>
            </div>
            <div className="space-y-1.5 p-2 rounded-md bg-secondary/5 border border-secondary/10">
              <label className="text-xs text-muted-foreground uppercase tracking-wider">Reverb</label>
              <Slider value={reverb} min={0} max={1} step={0.01} onChange={handleReverb} />
              <div className="text-center text-sm font-mono text-secondary font-bold">{reverb.toFixed(2)}</div>
            </div>
            <div className="space-y-1.5 p-2 rounded-md bg-secondary/5 border border-secondary/10">
              <label className="text-xs text-muted-foreground uppercase tracking-wider">Echo</label>
              <Slider value={echo} min={0} max={1} step={0.01} onChange={handleEcho} />
              <div className="text-center text-sm font-mono text-secondary font-bold">{echo.toFixed(2)}</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1.5">
              ...
              <Select value={key} onChange={handleKeyChange} className="bg-accent/10 border-accent/30 text-accent font-bold">
                {KEYS.map(k => (
            ...
            <div className="space-y-1.5">
              ...
              <Select value={scale} onChange={handleScaleChange} className="bg-accent/10 border-accent/30 text-accent font-bold">
                {SCALES.map(s => (
