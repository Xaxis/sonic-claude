    ...
    ({ title, headerActions, showHeader = true, className, children, ...props }, ref) => {
        return (
            <div
                ref={ref}
                className={cn(
                    "flex flex-col overflow-hidden bg-card/20 border border-primary/5",
                    className
                )}
                {...props}
            >
                {showHeader && (title || headerActions) && (
                    <div className="flex items-center justify-between border-b border-primary/5 bg-card/10 px-3 py-2">
                        {title && (
