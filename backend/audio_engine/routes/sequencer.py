...
def set_sequencer_service(service: SequencerService):
    """Inject sequencer service"""
    global _sequencer_service
    _sequencer_service = service
    logger.info("Sequencer service injected into routes")
...
async def get_sequences():
    ...
    sequences = _sequencer_service.get_all_sequences()
    return [
        SequenceResponse(
            id=seq.id,
            name=seq.name,
...
async def get_sequence(sequence_id: str):
    ...
    sequence = _sequencer_service.get_sequence(sequence_id)
    if not sequence:
...
async def delete_sequence(sequence_id: str):
    ...
    try:
        await _sequencer_service.delete_sequence(sequence_id)
...
async def add_midi_clip(sequence_id: str, request: AddMIDIClipRequest):
    ...
    try:
        ...
        clip_id = _sequencer_service.add_clip(
...
async def add_audio_clip(sequence_id: str, request: AddAudioClipRequest):
...
async def remove_clip(sequence_id: str, clip_id: str):
    """Remove clip from sequence"""
    if not _sequencer_service:
        raise HTTPException(status_code=503, detail="Sequencer service not initialized")

    try:
        _sequencer_service.remove_clip(sequence_id, clip_id)
...
async def update_clip(sequence_id: str, clip_id: str, request: UpdateClipRequest):
    ...
    try:
        _sequencer_service.update_clip(
            sequence_id=sequence_id,
            clip_id=clip_id,
            start_time=request.start_time,
            duration=request.duration,
...
async def play_sequence(sequence_id: str, request: PlaybackControlRequest = PlaybackControlRequest()):
    """Start playback of sequence"""
    if not _sequencer_service:
        raise HTTPException(status_code=503, detail="Sequencer service not initialized")

    try:
        await _sequencer_service.play(sequence_id, request.position)
...
async def pause_playback():
    ...
    await _sequencer_service.pause()
...
async def resume_playback():
    ...
    await _sequencer_service.resume()
...
async def set_tempo(request: SetTempoRequest):
    ...
    _sequencer_service.set_tempo(request.tempo)
...
async def seek_position(request: SeekRequest):
    ...
    _sequencer_service.seek(request.position)
...
async def get_playback_state():
    ...
    state = _sequencer_service.get_playback_state()
