...
_effects_service: Optional[EffectsService] = None


def set_effects_service(service: EffectsService):
    """Inject effects service"""
    global _effects_service
    _effects_service = service
    logger.info("Effects service injected into routes")
...
async def get_effectdefs_by_type(effect_type: str):
    ...
    try:
        ...
        effectdefs = _effects_service.get_effectdefs_by_type(effect_type_enum)
        return [
            EffectDefResponse(
                name=ed.name,
                effect_type=ed.effect_type.value,
...
async def create_effect(request: CreateEffectRequest):
    """Create a new effect instance"""
    if not _effects_service:
        raise HTTPException(status_code=503, detail="Effects service not initialized")
    
    try:
        effect = await _effects_service.create_effect(
...
async def free_effect(effect_id: int):
    """Free an effect"""
    if not _effects_service:
        raise HTTPException(status_code=503, detail="Effects service not initialized")
    
    await _effects_service.free_effect(effect_id)
    return {"status": "freed", "effect_id": effect_id}


@router.put("/effects/{effect_id}")
async def update_effect(effect_id: int, request: UpdateEffectRequest):
    """Update effect parameter"""
    if not _effects_service:
        raise HTTPException(status_code=503, detail="Effects service not initialized")
    
    try:
        await _effects_service.set_parameter(effect_id, request.parameter, request.value)
