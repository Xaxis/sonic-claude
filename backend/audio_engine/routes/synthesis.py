...
def set_synthesis_service(service: SynthesisService):
    """Inject synthesis service"""
    global _synthesis_service
    _synthesis_service = service
    logger.info("Synthesis service injected into routes")
...
async def get_synthdefs_by_category(category: str):
    """Get SynthDefs by category"""
    if not _synthesis_service:
        raise HTTPException(status_code=503, detail="Synthesis service not initialized")
    
    synthdefs = _synthesis_service.get_synthdefs_by_category(category)
    return [
        SynthDefResponse(
            name=sd.name,
            category=sd.category,
...
async def create_synth(request: CreateSynthRequest):
    ...
    try:
        synth = await _synthesis_service.create_synth(
...
async def free_synth(synth_id: int):
    """Free a synth"""
    if not _synthesis_service:
        raise HTTPException(status_code=503, detail="Synthesis service not initialized")
    
    await _synthesis_service.free_synth(synth_id)
    return {"status": "freed", "synth_id": synth_id}


@router.put("/synths/{synth_id}")
async def update_synth(synth_id: int, request: UpdateSynthRequest):
    """Update synth parameter"""
    if not _synthesis_service:
        raise HTTPException(status_code=503, detail="Synthesis service not initialized")
    
    try:
        await _synthesis_service.set_parameter(synth_id, request.parameter, request.value)
